...
```
- 使用`wget`命令下载 **Aria2 Pro**，最新版本见[Github](https://github.com/P3TERX/Aria2-Pro-Core/releases/)
```c
...
```
- 下载配置，这里采用[P3TERX](https://github.com/P3TERX)完美配置，项目地址见[Github](https://github.com/P3TERX/aria2.conf)
```c
https://github.com/P3TERX/aria2.conf/archive/refs/heads/master.zip
```
- 将配置解压，根据需求修改配置文件`aria2.conf`，并将目录内所有文件上传至`/opt/aria2/config`，`aria2.conf`示例如下
```c
#
...
# 从会话文件中读取下载任务
input-file=/opt/aria2/config/aria2.session

# 会话文件保存路径
# Aria2 退出时或指定的时间间隔会保存`错误/未完成`的下载任务到会话文件
save-session=/opt/aria2/config/aria2.session
...
dht-file-path=/opt/aria2/config/dht.dat

# IPv6 DHT 文件路径，默认：$HOME/.aria2/dht6.dat
dht-file-path6=/opt/aria2/config/dht6.dat
...
on-download-stop=/opt/aria2/config/delete.sh

# 下载完成后执行的命令
# 此项未定义则执行 下载停止后执行的命令 (on-download-stop)
on-download-complete=/opt/aria2/config/clean.sh
